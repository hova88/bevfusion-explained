 ...
 for(const width of [320,390,768,1440]){
  ...
  for(const [i,name] of ['00 Frame SYNC','01 Image ENCODE','02 Lift LIFT','03 Pool REDUCE','04 LiDAR VOXELIZE','05 Align REGISTER','06 Fuse FUSE','07 Head DECODE'].entries()){
   ...
   const audit=await page.evaluate(()=>{
    const root=document.querySelector('.stage-story');
    ...
    return {overflow:document.documentElement.scrollWidth>innerWidth,headingObscured:root.querySelector('h3').getBoundingClientRect().top<document.querySelector('.stage-nav').getBoundingClientRect().bottom,tiny,clipped,missingImages:[...document.images].filter(i=>i.complete&&i.naturalWidth===0).map(i=>i.src)};
   });
